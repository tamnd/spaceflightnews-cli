...
describes what users get. Pick a format with `-o`, or let spaceflightnews choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
spaceflightnews <command> -o table   # aligned columns for reading
spaceflightnews <command> -o jsonl   # one JSON object per line, for piping
spaceflightnews <command> -o json    # a single JSON array
spaceflightnews <command> -o csv     # spreadsheet friendly
spaceflightnews <command> -o tsv     # tab-separated
spaceflightnews <command> -o url     # just the URL column
spaceflightnews <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
spaceflightnews <command> --fields id,title,url
```
...
```bash
spaceflightnews <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
spaceflightnews <command>            # a table, because this is a terminal
spaceflightnews <command> | wc -l    # JSONL, because this is a pipe
```
